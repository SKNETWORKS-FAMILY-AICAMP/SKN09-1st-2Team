...
## 🚗 국내 친환경 자동차(전기 & 하이브리드) 현황 및 데이터 통합 플랫폼
...
### 🚀 프로젝트 필요성
![필요성 2](https://github.com/user-attachments/assets/49ce77e9-b7a7-456b-821c-ea900354127b)
![필요성 1](https://github.com/user-attachments/assets/e17b7ee0-f306-4bf5-abdc-8b488b1444ae)
![필요성 3](https://github.com/user-attachments/assets/f38fd544-f92e-4c63-92e8-3af90636e014)
...
| FAQ 크롤링                 | 01-07      | 01-07      | ALL                      | csv, .xlsx, json       | Streamlit 작업         |
| 5사 수요데이터 수집        | 01-07      | 01-07      | ALL                      | csv, .xlsx, json       | Streamlit 작업         |
...
|------|---|---|---|---|
|RQ-01|공통|DB 수집|시스템은 국내 5개사의 자동차별 월별 판매량 데이터를 수집한다. 시스템은 국내 5개사의 FAQ 데이터를 수집한다. 시스템은 자동으로 데이터를 업데이트하고 관리한다.|01/07|
|RQ-02|공통|DB 서버 구축|시스템은 수집된 데이터를 자동으로 DB 서버에 등록한다.|01/07|
|RQ-03|공통|Streamlit - 페이지|Streamlit 화면 구성 : 홈 화면, 등록현황, 차량 수요, FAQ 페이지를 구축한다. |01/07|
|RQ-04|공통|Streamlit - 페이지|홈 화면 구축 : 시스템은 프로젝트 개요 및 간략화 된 정보를 제공한다.|01/07|
|RQ-05|공통|Streamlit - 페이지|등록현황 화면 구축 : 시스템은 자동차 등록현환 데이터를 불러와서 친환경 자동차에 대한 내용을 시각화 한다.|01/07|
|RQ-06|공통|Streamlit - 페이지|차량수요 화면 구축 : 시스템은 프로젝트 개요 및 간략화 된 정보를 제공한다.|01/07|
|RQ-07|공통|Streamlit - 페이지|메뉴바 생성 : 사용자는 좌측의 메뉴바를 통해 각 페이지에 접근할 수 있다.|01/07|
|RQ-08|공통|등록현황 시각화|시스템은 지도 기반 지역별 등록 현황 및 막대 그래프를 통해 연료별 등록현을 제공한다.|01/08|
|RQ-09|공통|수요현황 시각화|시스템은 월별 수요 데이터에 대한 파이 그래프를 통해 제공한다.|01/08|
|RQ-10|공통|Filter 기능|사용자는 키워드 검색을 통해 FAQ에 접근할 수 있다.  사용자는 브랜드 선택을 통해 FAQ에 접근할 수 있다. 사용자는 카테고리 선택을 통해 FAQ에 접근할 수 있다.|01/07|
|RQ-11|공통|통합 FAQ 기능|시스템은 국내 5개사의 통합 데이터를 제공한다.|01/08|
...
- **김우중**:  
  크롤링 경로 찾는게 어려워 관련된 방법을 찾아봐야 할 것 같습니다.
